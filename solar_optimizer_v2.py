...
NGEN_PROFILE_EXPORT_KW = 8.90 # Nastavitev v NGEN profilu za oddajo
...
# Realistični faktorji umeritve po mesecih
MONTHLY_FACTORS = {
...
st.caption(f"v2.9: Meja odločbe {GRID_LEGAL_LIMIT_KW} kW | Mesečna umeritev: {CALIBRATION_BOOST:.2f}")
...
# Realna popoldanska poraba (0.5 - 0.6 kW)
base_load = [0.6 if 10 <= h <= 17 else 0.4 for h in range(24)]

# 4. 5-minutna simulacija
...
while sim_t < 21.0:
    ...
    # Preverjanje presežka nad mejo odločbe (9,4 kW)
...
if not spike_detected:
    ...
    st.markdown(f"""
    * **Ni nevarnosti za preboj soglasja (9,4 kW):** Baterija bo kmalu polna{full_info}. Preostanek sonca po napolnitvi ne bo presegel dovoljene oddaje.
    * **Končno stanje NGEN baterije:** **100 %**.
    * **Ukrep:** Brez preklapljanja profilov in brez potrebe po vklopu avtomobila.
    ...
    st.error(f"### ⚠️ NEVARNOST PREKORAČITVE 9,4 kW med {sp_s_h:02d}:{sp_s_m:02d} in {sp_e_h:02d}:{sp_e_m:02d}!")
    ...
    * **OBVEZNO ob {sp_e_h:02d}:{sp_e_m:02d} preklopite nazaj** na: **Hiša ➔ Baterija ➔ Omrežje**.
    """)
...
st.line_chart(active_chart[["Proizvodnja sonca (kW)", "Oddaja v omrežje (kW)", "Meja odločbe (9.4 kW)"]])
